...
export default function CommentScreen() {
  ...
  const getData = async () => {
    try {
      ...
      const notificaciones = querySnapshot.docs.map(
        (doc) => doc.data() as Notificacion
      );
      ...
      querySnapshot.docs.forEach((docSnapshot) => {
        ...
        if (!notificacion.leida) {
          ...
          setDoc(notificacionRef, { leida: true }, { merge: true })
            .then(() =>
              console.log(`Notificación ${docSnapshot.id} marcada como leída`)
            )
            .catch((error) =>
              console.error(
                `Error al actualizar la notificación ${docSnapshot.id}: `,
                error
              )
            );
        }
    ...
    <View key={key} style={[{ paddingTop: insets.top }]}>
      ...
      >
        {notificaciones.map((notificacion, index) => {
          ...
          console.log(
            `Notificación ${index} leída:`,
            notificacion.leida,
            notificacion.idUsuario
          ); // Esto imprimirá si cada notificación está leída o no
